    ...
    return (
        <header className="absolute top-0 w-full p-4 md:p-6 flex justify-between items-center z-20 animate-fade-in-up">
            <Link href="/" className="group flex items-center gap-2 shrink-0">
                <div className="font-black font-extrabold text-lg sm:text-xl md:text-2xl tracking-tight flex items-center gap-1 sm:gap-2">
            ...
            {showAuthLinks && (
                <nav className="flex gap-2 sm:gap-4 items-center ml-2">
                    {auth?.user ? (
                        ...
                        <>
                            <Link href={route('login')} className="text-blue-700 hover:text-blue-200 transition font-semibold text-xs sm:text-sm md:text-base px-1 whitespace-nowrap">
                                Sign In
                            </Link>
