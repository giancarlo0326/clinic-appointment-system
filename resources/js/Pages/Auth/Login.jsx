            ...
            <div className="w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl z-10 mt-20">
                ...
                <form onSubmit={submit} className="space-y-6">
                    <div>
                        ...
                        <input
                            type="email"
                            value={data.email}
                            className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-2xl text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white/20 transition-all duration-300"
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-1.5 ml-1">
                            ...
                            {canResetPassword && (
                                <Link href={route('password.request')} className="text-xs text-blue-300 hover:text-white transition-colors">
                                    Forgot password?
                                </Link>
                            )}
                        </div>
                    ...
                    <button
                        type="submit"
                        disabled={processing}
                        className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-xl hover:bg-blue-500 hover:shadow-blue-500/20 transition-all duration-300 active:scale-[0.98] disabled:opacity-50 tracking-wider uppercase text-sm"
                    >
                ...
                <div className="mt-8 text-center border-t border-white/10 pt-6">
                    <p className="text-sm text-blue-50/70">
                        New to Clinicare? {' '}
                        <Link href={route('register')} className="text-white font-bold hover:text-blue-400 underline decoration-blue-500/50 underline-offset-4 transition-all">
                            Create an account
                        </Link>
                    </p>
                </div>
